...
function Inner() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const res = await fetch("/api/v1/company/verification/confirm", {
        method: "POST",
      ...
      if (!res.ok) {
        setOk(false);
        setMsg(j?.error?.message ?? "Confirmação falhou.");
        return;
      }
      setOk(true);
      setMsg(j?.data?.message ?? "Email confirmado.");
    })();
  }, [token]);

  return (
    <div className="mx-auto max-w-md px-4 py-16 text-center">
      <h1 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Verificação de empresa</h1>
